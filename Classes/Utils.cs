...
{
    public static DateTime DosDateToDateTime(uint dosDateTime)
    {
    ...
    public static byte[]? GetBytes(byte[]? data, int offset, int length)
    {
        ...
        if (offset < 0 || offset + length > data.Length)
            throw new InvalidDataException(
                $"Requested {length} bytes at offset {offset}, but data is only {data.Length} bytes long.");
        if (length < 0) length = 0;
        var templist = new byte[length];

        if (offset + length > data.Length) length = data.Length - offset;

        if (length <= data.Length && length >= 0) Buffer.BlockCopy(data, offset, templist, 0x00, length);
    ...
    {
        var bytes = GetBytes(data, offset, length);
    ...
    public static byte[]? StringToByteArray(string hex)
    {
        if (!hex.All("0123456789abcdefABCDEF".Contains))
            throw new InvalidDataException("String is not a valid hex string");
    ...
    {
        var hash = new HMACSHA1(key).ComputeHash(salt);
        return GetBytes(hash, 0, 0x10);
    }
    ...
    public static bool IsCpuKeyValid(byte[]? key, bool fixKey = false)
    {
        ...
        var bitArray = new BitArray(hammingArray); //array of true/false for 0/1 ez


        foreach (bool s in bitArray)
            if (s)
                hamming++; //if true, increase hamming
