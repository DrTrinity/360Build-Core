...
public class Patchslot : IEnumerable<Bootloader>
{
    public Patchslot(byte[] data, int offset, int length = 0x10000)
    {
        try
        {
            ...
            CgSg = Bootloader.Create(data, offset + CfSf.Length,
                (Bootloader.BootloaderType)Utils.GetInt(data, offset + CfSf.Length, 2));
        }
        catch (InvalidBootloaderException e)
        {
            throw new InvalidPatchslotException($"A patchslot is invalid due to invalid bootloader: {e.Message}");
        }
    }
